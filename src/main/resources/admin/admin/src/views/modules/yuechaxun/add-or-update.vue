<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="会员余额" prop="huiyuanyue">
          <el-input v-model="ruleForm.huiyuanyue" 
              placeholder="会员余额" clearable  :readonly="ro.huiyuanyue"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="会员余额" prop="huiyuanyue">
              <el-input v-model="ruleForm.huiyuanyue" 
                placeholder="会员余额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    zhanghaoChange () {
      this.$http({
        ...
        if (data && data.code === 0) {
              if(data.data.huiyuankahao){
                this.ruleForm.huiyuankahao = data.data.huiyuankahao
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuechaxun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuechaxunCrossAddOrUpdateFlag = false;
                  this.parent.search();
